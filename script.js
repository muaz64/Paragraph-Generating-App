...
function shuffle(array) {
  let arrayCopy = [...array]; // Make a copy of the array
...
function generate() {
  if (item.value == 0) {
      ...
      alert(`The value cannot be greater than ${paragraphs.length}`);
  } else {
      const shuffledParagraphs = shuffle(paragraphs);
      const selectedParagraphs = shuffledParagraphs.slice(0, item.value);
